...
void Usage() {
  printf(
      ...
      "vertex.\n"
      "                          If -1: run full exact BC using BGL (see "
      "issue#251).\n"
 ...
 * @param[in] source_path Pointer to a vector to store CPU computed labels for
 * each node
 * @param[in] src VertexId of source node if there is any
 * @param[in] quiet Don't print out anything to stdout
 */
template <typename VertexId, typename SizeT, typename Value>
void ReferenceBC(const Csr<VertexId, SizeT, Value> &graph, Value *bc_values,
                 Value *sigmas, VertexId *source_path, VertexId src,
                 bool quiet = false) {
...
cudaError_t RunTests(Info<VertexId, SizeT, Value> *info) {
  ...
  Value *reference_check_sigmas =
      (quick_mode || (src == -1)) ? NULL : reference_sigmas;
  VertexId *reference_check_labels =
      (quick_mode || (src == -1)) ? NULL : reference_labels;
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    ...
    if (retval =
            util::GRError(problem->Reset(0, enactor->GetFrontierType(),
    ...
    for (VertexId i = start_src; i < end_src; ++i) {
      if (retval =
              util::GRError(problem->Reset(i, enactor->GetFrontierType(),
                            ...
                            "BC Problem Data Reset Failed", __FILE__, __LINE__))
        return retval;
      if (retval = util::GRError(enactor->Reset(), "BC Enactor Reset failed",
                                 __FILE__, __LINE__))
        return retval;
      for (int gpu = 0; gpu < num_gpus; gpu++) {
        if (retval = util::SetDevice(gpu_idx[gpu])) return retval;
...
//                   reference_check_labels, src, quiet_mode);
  ...
  // Verify the result
  if (!quick_mode) {
    // util::cpu_mt::PrintCPUArray<SizeT, Value>("reference_check_bc_values",
    // reference_check_bc_values, graph->nodes);
    // util::cpu_mt::PrintCPUArray<SizeT, Value>("bc_values", h_bc_values,
    // graph->nodes);
    if (!quiet_mode) {
  ...
  if (reference_check_sigmas != NULL) {
    ...
    int num_error = CompareResults(h_sigmas, reference_check_sigmas,
                                   graph->nodes, true, quiet_mode);
    if (num_error > 0) {
      if (!quiet_mode) {
        printf("Number of errors occurred: %d\n", num_error);
  ...
  if (reference_check_labels != NULL) {
    ...
    int num_error = CompareResults(h_labels, reference_check_labels,
                                   graph->nodes, true, quiet_mode);
    if (num_error > 0) {
      if (!quiet_mode) {
        printf("Number of errors occurred: %d\n", num_error);
  ...
  /*if (!quiet_mode)
  {
      printf("\n\tMemory Usage(B)\t");
      for (int gpu = 0; gpu < num_gpus; gpu++)
          if (num_gpus > 1) {if (gpu != 0) printf(" #keys%d,0\t #keys%d,1\t
  #ins%d,0\t #ins%d,1", gpu, gpu, gpu, gpu); else printf(" #keys%d,0\t
  #keys%d,1", gpu, gpu);} else printf(" #keys%d,0\t #keys%d,1", gpu, gpu); if
  (num_gpus > 1) printf(" #keys%d", num_gpus); printf("\n"); double
  max_queue_sizing_[2] = {0, 0}, max_in_sizing_ = 0; for (int gpu = 0; gpu <
  num_gpus; gpu++)
      {
          ...
          for (int i = 0; i < num_gpus; i++)
          {
              ...
              {
                  SizeT x =
  problem->data_slices[gpu]->frontier_queues[i].keys[j].GetSize(); printf("\t
  %lld", (long long) x); double factor = 1.0 * x / (num_gpus > 1 ?
  problem->graph_slices[gpu]->in_counter[i] :
  problem->graph_slices[gpu]->nodes); if (factor > max_queue_sizing_[j])
  max_queue_sizing_[j] = factor;
              ...
              if (num_gpus > 1 && i != 0 )
                  ...
                  {
                      SizeT x =
  problem->data_slices[gpu][0].keys_in[t][i].GetSize(); printf("\t %lld", (long
  long) x); double factor = 1.0 * x / problem->graph_slices[gpu]->in_counter[i];
  ...
  max_queue_sizing_[1]); if (num_gpus > 1) printf("\t in_sizing =\t %lf",
  max_in_sizing_); printf("\n");
  }
  */
